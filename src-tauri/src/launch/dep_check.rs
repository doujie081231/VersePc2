...
/// 当前操作系统版本字符串（形如 "10.0.22621"），非 Windows 返回空串。
/// 从注册表读取 Windows NT 版本号，用于匹配版本 JSON 中 libraries 的 os.version 规则。
static OS_VERSION: LazyLock<String> = LazyLock::new(|| {
    #[cfg(target_os = "windows")]
    {
            ...
            .open_subkey(r"SOFTWARE\Microsoft\Windows NT\CurrentVersion")
        {
            ...
            if let (Some(ma), Some(mi), Some(b)) = (major, minor, build) {
                return format!("{}.{}.{}", ma, mi, b);
            }
        }
        String::new()
    }
...
/// 判断系统版本是否匹配 rules 中 os.version 给定的正则模式。
/// 模式非法时视为匹配（不影响该库的判定）。非 Windows 或无版本信息返回 false。
                    ...
                    if let Ok(index_data) = serde_json::from_str::<Value>(&content) {
                        if let Some(objects) = index_data.get("objects").and_then(|v| v.as_object()) {
                            ...
                            for (name, info) in objects {
                                ...
                                let expected_size = info.get("size").and_then(|v| v.as_u64()).unwrap_or(0);
                                let sub_dir = &hash[..2.min(hash.len())];
                                let asset_path = assets_dir.join("objects").join(sub_dir).join(&hash);

                                let mut found = asset_exists_ok(&asset_path, expected_size);
                                if !found {
                                    if let Some(ext_assets) = &external_assets_dir {
                                        let ext_asset = ext_assets.join("objects").join(sub_dir).join(&hash);
                                        if asset_exists_ok(&ext_asset, expected_size) {
                                            found = true;
                                        }
                                    }
                                }
...
pub(crate) fn evaluate_rules(rules: &Vec<Value>, has_custom_resolution: bool) -> bool {
    ...
    for rule in rules {
        ...
        if let Some(os) = rule.get("os").and_then(|v| v.as_object()) {
            ...
            if let Some(os_ver) = os.get("version").and_then(|v| v.as_str()) {
                if !os_ver.is_empty() {
                    rule_matched = rule_matched && os_version_matches(os_ver);
                }
            }
        }
...
/// 判断资源文件是否完好：存在且（未声明大小 或 实际大小一致）。
/// 大小校验与资源缺失检测中的判定一致。
fn asset_exists_ok(path: &Path, expected_size: u64) -> bool {
    match std::fs::metadata(path) {
        Ok(m) => m.is_file() && (expected_size == 0 || m.len() == expected_size),
        Err(_) => false,
    }
}
...
fn push_missing_main_jar_with_fallback(result: &mut DepCheckResult, version_json: &Value, version_id: &str, versions_dir: &Path) {
    ...
    // 优先使用当前版本自身声明的 client 下载信息（自包含合并版本移除了 inheritsFrom，
    // 但合并时保留了 downloads.client，缺主 jar 时可直接据此下载到本版本目录）
    if let Some(client) = version_json.get("downloads").and_then(|v| v.get("client")) {
        ...
        if !url.is_empty() {
            ...
            fallback_jar_id = version_id.to_string();
        }
    }

    // 自身无 client 下载信息时，沿 inheritsFrom 链查找前置版本的 client 下载信息
    let mut visited = std::collections::HashSet::new();
    let mut cur_json = version_json.clone();
    let mut cur_inherits = utils::get_str(&cur_json, "inheritsFrom");
    while fallback_jar_id.is_empty() && !cur_inherits.is_empty() && !visited.contains(&cur_inherits) {
