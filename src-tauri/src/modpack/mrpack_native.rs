...
async fn run_import_main(
    ...
    // 目标主 jar：优先 jar/继承版本，自包含合并版本则用自身版本目录。
    // 自包含版本移除了 inheritsFrom，但合并时保留了 downloads.client，据此直接下载主 jar。
    if let Some(ref merged) = merged_json {
        ...
        if !main_jar_path.exists() {
            let jar_url = merged
                ...
                .map(|s| s.to_string())
                .or_else(|| {
                    // 继承版本自身通常无 client，读基础版本 JSON 的 client url
                    if main_jar_id != version_id {
                        let base_json_path =
                            versions_dir().join(&main_jar_id).join(format!("{}.json", main_jar_id));
    ...
    if let Some(loader_obj) = loader_json.as_object() {
        for (key, loader_val) in loader_obj {
            ...
            if key == "arguments" && loader_val.get("arguments").is_some() && base_json.get("arguments").is_some() {
                ...
                // 合并 jvm 参数：以“flag+值”成对为单位去重，避免把同名成对 flag(如 --add-opens)折叠
                let mut merged_jvm: Vec<Value> = base_args
                    ...
                    .unwrap_or_default();
                {
                    let base_groups = group_jvm_tokens(&merged_jvm);
                    for g in group_jvm_tokens(&expanded_loader_jvm) {
                        let same: Vec<String> = g.iter().map(value_to_arg_string).collect();
                        let exists = base_groups.iter().any(|bg| {
                            bg.iter().map(value_to_arg_string).collect::<Vec<String>>() == same
                        });
                        if !exists {
                            for it in &g {
                                merged_jvm.push(it.clone());
                            }
                        }
                    }
                }
...
/// 把参数数组按“flag + 紧随其后的非 - 值”分组。
/// 用于保证 --add-opens/--add-exports 等成对参数在合并去重时不会把 flag 单独折叠掉，
/// 避免 flag 与值数量错配（漏掉成对 flag）。
fn group_jvm_tokens(args: &[Value]) -> Vec<Vec<Value>> {
    ...
    while i < args.len() {
        ...
        if let Some(s) = cur.as_str() {
            if s.starts_with('-') {
                // flag：收集其后紧邻的非 '-' 值，作为一个成对组
                let mut group: Vec<Value> = vec![cur.clone()];
                let mut j = i + 1;
                while j < args.len() {
                    if let Some(ns) = args[j].as_str() {
                        if !ns.starts_with('-') {
                            group.push(args[j].clone());
                            j += 1;
                            continue;
                        }
                    }
...
mod tests {
    ...
    #[test]
    fn group_preserves_pair_flags() {
        // Forge 类版本常见的多个 --add-opens：必须保持成对，不能折叠成 flag 少值多
        let args: Vec<Value> = serde_json::from_str(r#"[
            "--add-opens",
        ...
        .unwrap();
        let groups = group_jvm_tokens(&args);
        assert_eq!(groups.len(), 2, "应保留两个成对组，实际: {:?}", groups);
        for g in &groups {
            assert_eq!(g.len(), 2, "成对组应为 flag+值: {:?}", g);
            assert_eq!(g[0].as_str().unwrap(), "--add-opens");
        }
        assert_eq!(groups[1][1].as_str().unwrap(), "java.base/java.util=ALL-UNNAMED");
    }
}
