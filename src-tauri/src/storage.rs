...
/// 数据目录覆盖：由独立守护进程（verse-server-host）通过 --data-dir 注入，
/// 使服务器能共享主程序同一份 versions/libraries/servers 数据。
/// 主程序不调用 override，保持原解析逻辑不变。
static DATA_DIR_OVERRIDE: std::sync::OnceLock<Mutex<Option<std::path::PathBuf>>> = std::sync::OnceLock::new();

/// 让后续 resolve_data_dir 一律返回该目录（供守护进程注入共享数据目录）。
pub fn override_data_dir(path: std::path::PathBuf) {
    let m = DATA_DIR_OVERRIDE.get_or_init(|| Mutex::new(None));
    *m.lock().unwrap() = Some(path);
}

pub fn resolve_data_dir() -> std::path::PathBuf {
    // 守护进程注入的覆盖优先：直接使用主程序解析出的数据目录
    if let Some(m) = DATA_DIR_OVERRIDE.get() {
        if let Some(p) = m.lock().unwrap().as_ref() {
            return p.clone();
        }
    }
